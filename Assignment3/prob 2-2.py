...
class ProcessA(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            if self.file2.lock.locked():
                self.file1.lock.release() # release file1's lock
                print(Fore.LIGHTRED_EX + f"{self.name} detected deadlock in {self.file2.name}. Waiting for unlock...")
                time.sleep(random.random() * 3)
                continue

            # Step 3: Acquire file2's lock and write to it
...
class ProcessB(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            if self.file1.lock.locked():
                self.file2.lock.release() # release file2's lock
                print(Fore.LIGHTRED_EX + f"{self.name} detected deadlock in {self.file1.name}. Waiting for unlock...")
                time.sleep(random.random() * 3)
                continue

            # Step 3: Acquire file1's lock and write to it
